...
# from data_manager.utils import may_make_dir
...
def get_rank_list(dist_vec, q_id, q_cam, g_ids, g_cams, rank_list_size):
  ...
  same_id = []
  i = 0
  g_sorted_inds = [g_ids[ind] for ind in sort_inds]
  g_sorted_cams = [g_cams[ind] for ind in sort_inds]
  for ind, g_id, g_cam in zip(sort_inds, g_sorted_inds, g_sorted_cams):
